...
def few_shot(memory, token, class_name):
    ...
    # B D L
    M = 1 / 2 * torch.min(1.0 - torch.bmm(F.normalize(token.squeeze(2), dim=-1), F.normalize(retrive, dim=1)), dim=-1)[
        0]
    return M
...
def train(args, ):
    ...
    #print(type(matches))
    for i, (dataloader_test, classname) in enumerate(zip(dataloader_list_test, CLSNAMES)):
        ...
        image_head = image_head.eval()
        text_head = text_head.eval()
        ...
        table_data.append([classname, auroc_sp, ap_sp, f1_sp])
        AUROC.append(auroc_sp)
        AP.append(ap_sp)
        F1_MAX.append(f1_sp)
        print(f"{classname}", auroc_sp)
        print(f"{classname}", ap_sp)
        print(f"{classname}", f1_sp)
...
@torch.no_grad()
def evaluate(i, model, testloader, classname, text_features, image_head=None, text_head=None):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    origin_visual_features = []
    
    for idx, inputs in enumerate(tqdm(testloader)):
        images = inputs["image"].to(device)
        batch_size = images.shape[0]
        with torch.no_grad():
            large_scale_tokens, mid_scale_tokens, patch_tokens, class_tokens, large_scale, mid_scale = model.encode_image(
                images, patch_size)
        origin_visual_features.extend(class_tokens.detach().cpu())
        ...
        np_text_feature = np.array(text_feature.detach().cpu())
        ...
        global_visual_features.extend(class_tokens.detach().cpu())
    ...
    plt.savefig(save_path + "his.jpg")
    plt.close()
    gt_sp = np.array(results['gt_sp'])
    pr_sp = np.array(results['pr_sp'])
    
    visual_features = np.array(global_visual_features)
    origin_visual_features = np.array(origin_visual_features)
    np_label = np.array(results['gt_sp'])
    save_f_path = "./vis//"+ args.dataset + "//" + classname + "//" 
    if not os.path.exists(save_f_path):
        os.makedirs(save_f_path)
    np.save(save_f_path+ "visual_features.npy",visual_features)
    np.save(save_f_path+ "origin_visual_features.npy",origin_visual_features)
    np.save(save_f_path+ "text_features.npy",np_text_feature)
    np.save(save_f_path+ "label.npy",np_label)
