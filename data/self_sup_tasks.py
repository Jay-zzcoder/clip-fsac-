...
def patch_ex(ima_dest, ima_src=None, same=False, num_patches=1,
    ...
    if isinstance(ima_dest, PIL.Image.Image):
        ima_dest = np.array(input)
    """
    ...
    patchex = PIL.Image.fromarray(patchex)
...
if __name__ == '__main__':
    ...
    ima_dest = np.ones((100, 100, 3), dtype=np.uint8) * 255
    ima_src = np.random.randint(0, 255, size=(100, 100, 3), dtype=np.uint8)
    patchex, label = patch_ex(ima_dest, ima_src, num_patches=1)
    # 创建一个新的图形窗口
    fig, axes = plt.subplots(nrows=1, ncols=3)

    # 在第一个轴上显示图片1
    axes[0].imshow(ima_dest)
    axes[0].set_title('Image 1')

    # 在第二个轴上显示图片2
    axes[1].imshow(ima_src)
    axes[1].set_title('Image 2')

    # 在第三个轴上显示图片3
    axes[2].imshow(patchex)
